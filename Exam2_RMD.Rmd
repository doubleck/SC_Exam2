...
pkyp<-c(12, 15, 42, 52, 59, 73,82, 91, 96, 105, 114, 120, 121, 128, 130, 139, 139, 157)  ##The months that kyphosis present.
pkyp<-t(rbind(a.18,pkyp)) ##Combine 1 and months that kyphosis present.
...
Z<-cbind(rep(1,ncol(kyp)),kyp[,2])  ##(Prepare independent variable with column with value (1) to be calculated )
```
...
```{r}
find_update<- function(Z, W, working_response){
...
##Create Data Frame for Using Predict Function
...
##Calculate Residual
residual<-abs(kyp.plot[,1]-kyp.plot[,3])

##Include Residual in Dataset
kyp.plot2<-cbind(kyp.plot,residual)

##GGPlot requires Data Frame
data.p <- as.data.frame(kyp.plot)

ggplot(data.p ,aes(x=months,y=kyphosis)) + geom_point() +
stat_smooth(method="glm",fullrange=TRUE)

## PLot X=Months Y=Probability with Fitted Line
qplot(residual,y, data=data.p)
...
# show us the current value of beta if "verbose" is TRUE
if(verbose==T){
msg <- paste0('Iteration ', counter, ': Beta = (', beta[1], ', ', beta[2], ',', beta[3], ')')
cat(msg)
cat('\n')
}
}
...
## Verify the reselt using package GLM
glm(kyp[,1]~kyp[,2]+kyp[,3], family=binomial)
